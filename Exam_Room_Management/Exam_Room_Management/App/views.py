...
def room_page(request):
    ...
    if ( 'id' in request.GET and request.GET['id'] and
        ...
        if ExamRoom.objects.filter(roomID=id_tmp) :
            ...
            add_info = ExamRoom(roomID=id_tmp,size=size_tmp,subA='Empty',subB='Empty')
            add_info.save()        
    else :
        errors.append('You must fill in all of the fields, if you want to add information.')
    return render_to_response('room_page.html',{'errors': errors,'rooms': ExamRoom.objects.order_by('roomID')})
...
def deleting_room_info_page(request, room_id):
    ...
    return render_to_response('room_page.html',{'rooms':ExamRoom.objects.order_by('roomID')}) 
